...
void beatTiming(int i) {
		if (beatDetected[i] == 2) { // if beat has been detected
			if (beatTimer[i] < 100) // and it isn't 200 ms from the last one
...
//
//const uint8_t numOfCriteria = 2;
//bool criteria[numOfCriteria] = { false };
//
//int beatCriteria(int i) {
//	for (int i = 0; i <= numOfCriteria; i++)
//		criteria[i] = false;
//
//	if (spectrumValue[i] > spectrumValueOld[i])
//		criteria[0] = true;
//	if (spectrumValue[i] > average[i] * 2.3*stDev[i])
//		criteria[1] = true;
//}
//
